...
function debugLog(event: string, details: Record<string, unknown> = {}): void {
  if (!process.env.CURSOR_PROXY_DEBUG) return;
...
interface ActiveBridge {
  ...
  execTraces: Map<string, ExecTrace>;
}
  ...
  void (async () => {
    ...
    try {
      while (true) {
        ...
        if (done) break;
        const message = decoder.decode(value, { stream: true }).trim();
          ...
          try {
            ...
            processServerMessage(
              ...
              (exec) => {
                ...
                execTraces.set(exec.execId, trace);
                debugLog("exec.emitted", { bridgeKey, ...trace });
...
function handleToolResultResume(
  ...
  for (const exec of pendingExecs) {
    ...
    const trace = execTraces.get(exec.execId);
    if (trace) {
      trace.resultReceivedAt = Date.now();
    ...
    debugLog("exec.result_frame", {
      ...
      mcpResultBytes: mcpResultBytes.length,
